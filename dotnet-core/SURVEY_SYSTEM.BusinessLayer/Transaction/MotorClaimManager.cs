...
namespace SURVEY_SYSTEM.BusinessLayer.Transaction
{
    public class MotorClaimManager
    {
        ...
        public DataTable FetchByClmUid(MotorClaim objMotorClaim)
        {
            try
            {
                string query = $"SELECT * FROM MOTOR_CLAIM WHERE CLM_UID = '{objMotorClaim.ClmUid}'";
        ...
        public bool CheckDuplicatePolRepNo(MotorClaim objMotorClaim, int? pClmUid = null)
        {
            try
            {
                string query;
                ...
                if (pClmUid.HasValue)
                {
                    query = $"SELECT CLM_POL_REP_NO FROM MOTOR_CLAIM WHERE CLM_POL_REP_NO = '{objMotorClaim.ClmPolRepNo}' AND CLM_UID != {pClmUid}";
                }
                else
                {
                    query = $"SELECT CLM_POL_REP_NO FROM MOTOR_CLAIM WHERE CLM_POL_REP_NO = '{objMotorClaim.ClmPolRepNo}'";
                }

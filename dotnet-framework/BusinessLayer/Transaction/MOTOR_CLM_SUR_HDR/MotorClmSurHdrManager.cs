...
namespace BusinessLayer
{
    public class MotorClmSurHdrManager
    {
        ...
        public string GetSurUidSequence()
        {
            try
            {
        ...
        public String GetSurUidBySurNo(MotorClmSurHdr objMotorClmSurHdr)
        {
            try
            {
                string query = $"SELECT SUR_UID FROM MOTOR_CLM_SUR_HDR WHERE SUR_NO = '{objMotorClmSurHdr.SurNo}'";
                string currency = Dbconnection.ExecuteScalar(query).ToString();
                return currency;
            }
            catch (Exception)
            {
